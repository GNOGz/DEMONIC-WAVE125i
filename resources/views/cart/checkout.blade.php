                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <textarea name="shipping_address" id="shipping_address" rows="4"
                            class="hidden mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $address->address_line ?? '' }}</textarea>
                        <x-input-label for="Address" :value="__('Address')" />
                        <x-text-input disabled=true id="address_detail" name="detail" type="text" placeholder="Detail"
                            class="mt-1 block w-full" :value="old('detail', $address?->detail ?? '')" required autofocus
                            autocomplete="detail" />
                        <x-input-error class="mt-2" :messages="$errors->get('detail')" />
                        <div class="flex flex-row justify-center items-center gap-3 mt-2">
                            <x-text-input disabled=true id="address_district" name="distric" type="text" placeholder="District"
                                class="mt-1 block w-full max-w-[9.5rem]" :value="old('distric', $address?->distric ?? '')" required autofocus autocomplete="distric" />
                            <x-input-error class="mt-2" :messages="$errors->get('distric')" />
                            <x-text-input disabled=true id="address_province" name="province" type="text" placeholder="Province"
                                class="mt-1 block w-full max-w-[9.5rem]" :value="old('province', $address?->province ?? '')" required autofocus autocomplete="province" />
                            <x-input-error class="mt-2" :messages="$errors->get('province')" />
                            <x-text-input disabled=true id="address_country" name="country" type="text" placeholder="Country"
                                class="mt-1 block w-full max-w-[9.5rem]" :value="old('country', $address?->country ?? '')" required autofocus autocomplete="country" />
                            <x-input-error class="mt-2" :messages="$errors->get('country')" />
                            <x-text-input disabled=true id="address_postal_code" name="postal_code" type="text"
                                placeholder="Postal Code" class="mt-1 block w-full max-w-[9.5rem]"
                                :value="old('postal_code', $address?->postal_code ?? '')" required autofocus
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                ...
                                <div class="flex justify-between items-center mt-2">
                                    ...
                                    <span class="text-sm font-medium">{{ number_format($shipping, 0, '.', ',') }}
                                        baht</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Right Column -->
                    <div class="bg-gray-50 rounded-lg p-6 absolute right-28 top-60">
                        ...
                        <div class="space-y-4 mb-6">
                            @foreach($cartItems as $item)
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        <div class="w-16 h-16 bg-gray-200 rounded overflow-hidden">
                                            ...
                                            @if($p && (isset($p->image_url) || isset($p->image)))
                                                ...
                                                @if($img)
                                                    <img src="{{ asset('images/products/' . ($p->category->category_name ?? 'default') . '/' . $img) }}"
                                    ...
                                    <span
                                        class="font-medium">{{ number_format($item->product->price * $item->quantity, 0, '.', ',') }}
                                        baht</span>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 flex items-center justify-center">
                            <button id="showQRButton" type="button"
                                class="bg-blue-600 text-white px-4 py-2 rounded-md hover:{{$isAddressComplete ? '' : ''  }} {{!$isAddressComplete ? 'bg-gray-500' : ''}}">
                                Checkout
                            </button>
                        </div>
                            ...
                            <div class="bg-white p-8 rounded-lg max-w-md w-full mx-4">
                                ...
                                <div class="space-y-4">
                                    <div class="flex justify-center">
                                        <img src="{{ asset('images/cart/db-qrcode.png') }}" alt="QR Code"
